      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-4">
        ...
        {type === 'mental' && (
          <div className={`p-5 rounded-2xl border ${riskColors[data.anxiety_level]}`}>
            <p className="text-xs font-medium uppercase tracking-wide mb-1 opacity-70">Anxiety level</p>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          ...
          <div className="flex items-end gap-1">
            <span className="text-4xl font-semibold text-gray-900">{mainScore}</span>
            <span className="text-gray-400 text-sm mb-1">/ 100</span>
          </div>
          <div className="mt-3 h-2 bg-gray-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${
                mainScore < 30 ? 'bg-green-500' : mainScore < 60 ? 'bg-yellow-500' : 'bg-red-500'
              }`}
        ...
        {/* Tips */}
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          <h3 className="text-sm font-medium text-gray-900 mb-4">Personalized recommendations</h3>
        ...
        {type === 'mental' && data.seek_help && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
            <p className="text-sm font-medium text-red-700 mb-1">Consider speaking with a professional</p>
            <p className="text-xs text-red-600">iCall helpline: 9152987821 — Free mental health support in India</p>
          </div>
        )}
