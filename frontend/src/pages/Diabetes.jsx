  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          const w = await getTodayWater(user.id)
      ...
      <div className="max-w-2xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className={`rounded-2xl p-4 col-span-1 ${
            ...
            <p className="text-xl font-semibold">{lastRecord?.risk_level ?? '—'}</p>
        ...
        {todayWater && (
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link to="/water" className="text-xs text-blue-600 hover:underline">Track</Link>
            </div>
            <div className="flex items-center gap-4">
              <div>
                <div className="flex items-end gap-1">
                  <span className="text-3xl font-semibold text-blue-600">{todayWater.glasses}</span>
                  <span className="text-gray-400 text-sm mb-1">/ {todayWater.goal}</span>
                </div>
                <p className="text-xs text-gray-500 mt-1">{todayWater.message}</p>
              </div>
        ...
        {chartData.length > 1 && (
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <h2 className="text-sm font-medium text-gray-900 mb-4">Risk score trend</h2>
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={chartData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
                <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ border: '1px solid #f3f4f6', borderRadius: '12px', fontSize: '12px' }} />
        ...
        {mentalChartData.length > 1 && (
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <h2 className="text-sm font-medium text-gray-900 mb-4">Mental health trend</h2>
            <ResponsiveContainer width="100%" height={160}>
            ...
            <div className="space-y-3">
              {healthHistory.slice(0, 3).map(item => (
                <div key={item.id}
                  ...
                  <div>
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${riskColors[item.risk_level]}`}>
                      {item.risk_level}
                    </span>
                  ...
                  <div className="text-right">
                    <p className="text-lg font-semibold text-gray-900">{item.risk_score}</p>
        ...
        {lastMental && (
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-medium text-gray-900">Last mental health check</h2>
              <Link to="/history" className="text-xs text-blue-600 hover:underline">See all</Link>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <p className="text-xs text-gray-500">Score</p>
                <p className="text-xl font-semibold text-gray-900">{lastMental.mental_health_score}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Depression</p>
                <p className="text-sm font-medium text-gray-900">{lastMental.depression_level}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Anxiety</p>
                <p className="text-sm font-medium text-gray-900">{lastMental.anxiety_level}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {healthHistory.length === 0 && mentalHistory.length === 0 && (
          <div className="bg-white border border-gray-100 rounded-2xl p-8 text-center">
            <p className="text-gray-400 text-sm mb-4">No health data yet. Run your first analysis!</p>
